...
class Nvib(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if prior_var is not None:
            self.prior_var = nn.Parameter(prior_var, requires_grad=learnable_prior)
        else:
            self.prior_var = nn.Parameter(torch.ones(size_in), requires_grad=learnable_prior)
    ...
    def forward(self, encoder_output, mask=None, **kwargs):
        ...
        alpha = torch.cat((self.alpha_activation(self.prior_log_alpha).repeat(B, 1, 1), alpha), 1)
        mask = (
